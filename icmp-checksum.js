...
export function buildEchoRequest(id, seq, payloadSize) {
...
export function parseEchoReply(buf) {
    if (buf.length < 28)
        return null; // minimum: 20 IP + 8 ICMP
    const ipHeaderLen = (buf[0] & 0x0F) * 4;
    const icmpOff = ipHeaderLen;
    if (buf.length < icmpOff + 8)
        return null;
...
//# sourceMappingURL=icmp-checksum.js.map