...
export class LinuxIcmpBackend implements IcmpBackend {
    name = "POSIX ICMP socket (Linux)";
    private koffi: any;
    private libc: any;
    private socketFn: any;
    private sendtoFn: any;
    private recvfromFn: any;
    private closeFn: any;
    private setsockoptFn: any;
    private inet_aton: any;
    private loaded = false;
    private loadError = "";
    private useRaw = false; // true if DGRAM failed and we escalated to RAW
    private diagMessages: string[] = [];
    ...
    private buildSockaddr(address: string) {
        ...
        const ok: number = this.inet_aton(address, addrBuf);
        if (!ok) return null;
        return buf;
    }
    ...
    async ping(address: string, options: Required<PingOptions>, seq: number): Promise<PingReply> {
        ...
        try {
            ...
            const received: number = this.recvfromFn(fd, recvBuf, 1500, 0, srcAddr, addrLen);
