...
export async function lookupMac(ipv4: string): Promise<string> {
    if (!isLocalSubnet(ipv4)) return "";
    const platform = os.platform();
    if (platform === "win32") return lookupMacWin32(ipv4);
    if (platform === "linux") return lookupMacLinux(ipv4);
    if (platform === "darwin") return lookupMacDarwin(ipv4);
    return "";
}

/** True iff the IP falls inside any non-loopback IPv4 subnet on this host. */
function isLocalSubnet(ipv4: string): boolean {
    ...
    for (const list of Object.values(os.networkInterfaces())) {
        ...
        for (const iface of list) {
            ...
            let match = true;
            for (let i = 0; i < 4; i++) {
                if ((target[i] & mask[i]) !== (addr[i] & mask[i])) { match = false; break; }
            }
...
let win32State: { SendARP: any } | null = null;

async function lookupMacWin32(ipv4: string): Promise<string> {
    try {
        if (!win32State) {
        ...
        const rc = await new Promise<number>((resolve, reject) => {
            win32State!.SendARP.async(destIp, 0, macBuf, lenBuf, (err: Error | null, result: number) => {
                if (err) reject(err);
                else resolve(result);
            });
        });
...
async function lookupMacLinux(ipv4: string): Promise<string> {
    try {
...
async function lookupMacDarwin(ipv4: string): Promise<string> {
    try {
...
function normalizeMac(raw: string): string {
    ...
    if (parts.some(p => !/^[0-9a-f]{1,2}$/i.test(p))) return "";
    const canonical = parts.map(p => p.padStart(2, "0").toLowerCase()).join(":");
    if (canonical === "00:00:00:00:00:00") return "";
    return canonical;
}
